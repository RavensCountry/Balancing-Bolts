...
def test_property_requires_manager_role():
    # Create a maintenance user (password must be at least 8 chars)
    r = client.post('/api/auth/signup', data={'name':'Maint','email':'maint@example.com','password':'password1','role':'maintenance'})
    ...
    r3 = client.post('/api/auth/signup', data={'name':'Mgr','email':'mgr2@example.com','password':'password1','role':'manager'})
    ...
    r5 = client.post('/api/auth/signup', data={'name':'Admin','email':'admin@example.com','password':'password1','role':'admin'})
